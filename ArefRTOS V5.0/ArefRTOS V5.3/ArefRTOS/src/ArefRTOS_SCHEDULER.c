...
/***********************************************************
* 		   QUEUE MESSAGE BOX FUNCTIONS DEFINITION		   *
************************************************************/
#if QUEUE_MESSAGE_BOX_ENABLE == Enable
...
/***********************************************************
* 		  BINARY SEMAPHORE FUNCTIONS DEFINITION			   *
************************************************************/
#if BINARY_SAMPHORE_ENABLE == Enable
ArefRTOS_ErrorID ArefRTOS_AcquireBinarySemaphore(ArefRTOS_BinarySemaphore *pSemaphore)
{
    // 01. Initialize the local error code variable to NoError.
    ArefRTOS_ErrorID LOC_ArefRTOS_ErrorID = NoError;

    // 02. Check if the WaitingMessagesCounter in the binary semaphore's private data is 0.
    if (pSemaphore->QueuePrivateData.WaitingMessagesCounter == Reset)
    {
        // Step 3: If it's 0, increment the WaitingMessagesCounter to 1.
        pSemaphore->QueuePrivateData.WaitingMessagesCounter++;
    }
    // 03. Check if the WaitingMessagesCounter is already 1, indicating that the semaphore is already acquired.
    else if (pSemaphore->QueuePrivateData.WaitingMessagesCounter == Set)
    {
        // 04. Set the local error code to INVALID_OPERATION since the semaphore is already acquired.
        LOC_ArefRTOS_ErrorID = INVALID_OPERATION;
    }
    else
    {
        // 05. This branch should not be reached under normal circumstances.
        // Set the local error code to INVALID_OPERATION as a fallback.
        LOC_ArefRTOS_ErrorID = INVALID_OPERATION;
    }

    // 06. Return the error code. It will be NoError if the semaphore was successfully acquired,
    // or INVALID_OPERATION if the semaphore was already acquired or an unexpected condition occurred.
    return LOC_ArefRTOS_ErrorID;
}

ArefRTOS_ErrorID ArefRTOS_ReleaseBinarySemaphore(ArefRTOS_BinarySemaphore *pSemaphore)
{
    // 01. Initialize the local error code variable to NoError.
    ArefRTOS_ErrorID LOC_ArefRTOS_ErrorID = NoError;
    ...
    if (pSemaphore->QueuePrivateData.WaitingMessagesCounter == Reset)
    {
        // 03. Set the local error code to INVALID_OPERATION since the semaphore is not acquired.
        LOC_ArefRTOS_ErrorID = INVALID_OPERATION;
    }
    // 04. Check if the WaitingMessagesCounter is already 1, indicating that the semaphore is acquired.
    else if (pSemaphore->QueuePrivateData.WaitingMessagesCounter == Set)
    {
        // 05. Decrement the WaitingMessagesCounter to 0, releasing the semaphore.
        pSemaphore->QueuePrivateData.WaitingMessagesCounter--;
    }
    else
    {
        // 06. This branch should not be reached under normal circumstances.
        // Set the local error code to INVALID_OPERATION as a fallback.
        LOC_ArefRTOS_ErrorID = INVALID_OPERATION;
    }

    // 07. Return the error code. It will be NoError if the semaphore was successfully released,
    // or INVALID_OPERATION if the semaphore was not acquired or an unexpected condition occurred.
    return LOC_ArefRTOS_ErrorID;
}

ArefRTOS_ErrorID ArefRTOS_GetBinarySemaphoreState(ArefRTOS_BinarySemaphore *pSemaphore, uint8_t* ptr_u8Flag)
{
    // 01. Initialize the local error code variable to NoError.
    ArefRTOS_ErrorID LOC_ArefRTOS_ErrorID = NoError;

    // 02. Check if the WaitingMessagesCounter in the binary semaphore's private data is 0 or 1.
    if (pSemaphore->QueuePrivateData.WaitingMessagesCounter == 0 || pSemaphore->QueuePrivateData.WaitingMessagesCounter == 1)
    {
        // 03. If the counter is 0 or 1, set the value of ptr_u8Flag to the counter's value.
        *ptr_u8Flag = pSemaphore->QueuePrivateData.WaitingMessagesCounter;
    }
    else
    {
        // 04. This branch should not be reached under normal circumstances.
        // Set the local error code to INVALID_OPERATION as a fallback.
        LOC_ArefRTOS_ErrorID = INVALID_OPERATION;
    }

    // 05. Return the error code. It will be NoError if the semaphore state was retrieved successfully,
    // or INVALID_OPERATION if an unexpected condition occurred.
    return LOC_ArefRTOS_ErrorID;
}
#endif
